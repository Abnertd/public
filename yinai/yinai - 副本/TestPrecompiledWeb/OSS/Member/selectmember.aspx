...
<script type="text/javascript">
	function member_add(obj){
	    $.ajax({
	        url: encodeURI("member_do.aspx?action=check_member&member_id="+ SelectedValue(MM_findObj(obj)) +"&timer="+Math.random()),
		    type: "get", 
		    global: false, 
		    async: false,
		    dataType: "html",
...
<table width="100%" border="0" cellspacing="0" cellpadding="5">
				<form action="selectmember.aspx" method="post" name="frm_sch" id="frm_sch" >
				  <tr bgcolor="#F5F9FC" >
					<td align="right"><span class="left_nav">搜索</span> 
					等级：<%=myMEM.GetMemberGradeHTML(member_grade, "member_grade")%>
					 <input type="text" name="keyword" size="70" id="keyword" onfocus="if(this.value=='输入昵称、邮箱、姓名、电话、手机进行搜索'){this.value='';}" value="<% =keyword %>"> <input type="submit" name="btn_sch" class="btn_01" id="btn_sch" value="搜索" /></td>
				  </tr>
				  </form>
				</table>
    <% =myMEM.SelectMember()%>
