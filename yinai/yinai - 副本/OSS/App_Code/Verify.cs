...
public class Verify
{
    ...
    private Color _BgColor = Color.LemonChiffon;
    private int _ImgW = 65;
    private int _ImgH = 32;
    ...
    public void CodeGen()
    {
        ...
        for (int i = 0; i < 10; i++)
        {
            ...
            objgraphice.DrawLine(new Pen(Color.LimeGreen), x1, y1, x2, y2);
        }

        //画图片的前景干扰点
        for (int i = 0; i < 300; i++)
        {
            int x = random.Next(Objbitmap.Width);
            int y = random.Next(Objbitmap.Height);
            Objbitmap.SetPixel(x, y, Color.FromArgb(random.Next()));
        }
        ...
        //字体颜色
        SolidBrush fcolor = new SolidBrush(Color.Green);

        objgraphice.DrawString(VerifyStr, font, fcolor, 3, 3);
