...
<div class="content_div">
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="content_table">
    <tr>
      <td class="content_title">店铺商品评价管理</td>
    </tr>
				...
				<form action="shop_evaluate_list.aspx" method="post" name="frm_sch" id="frm_sch" >
				  <tr bgcolor="#F5F9FC" >
					<td align="right">
					 ...
					 <input type="text" name="keyword" size="50" id="keyword" onfocus="if(this.value=='输入合同编号、供应商、评论人、产品名称进行搜索'){this.value='';}" value="<% =keyword %>"> <input type="submit" name="btn_sch" class="btn_01" id="btn_sch" value="搜索" /></td>
				  </tr>
				  </form>
				</table>
    </td></tr>
    <tr>
      <td class="content_content">
        ...
        jQuery("#list").jqGrid({
        url: 'shop_evaluate_do.aspx?action=list&<%=ReqURL %>',
			datatype: "json",
            colNames: ['ID', '订单编号','供应商','会员', "产品","服务评分","发货评分","产品评分","审核状态", "操作"],
            colModel: [
    ...
    <%--    <%if (Public.CheckPrivilege("532bf3c0-e243-4a10-a664-b764fa7c23f4"))
        ...
        <div style="margin-top:5px;">
        <input type="button" id="export" class="bt_orange" value="审核通过" onclick="location.href='shop_evaluate_do.aspx?action=audit&Shop_Evaluate_ID='+jQuery('#list').jqGrid('getGridParam','selarrrow');" /> 
        <input type="button" id="Button1" class="bt_orange" value="审核不通过" onclick="location.href='shop_evaluate_do.aspx?action=deny&Shop_Evaluate_ID='+jQuery('#list').jqGrid('getGridParam','selarrrow');" /> 
        </div>
        </form>
        <%} %>--%>
      </td>
