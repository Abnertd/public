...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if ((Orders_DeliveryStatus != 0 && Orders_DeliveryStatus != 6) || Orders_Status != 1)
        {
            Public.Msg("error", "错误信息", "所选订单正在发货中", false, "{back}");
        }

        if (myApp.CheckProductStockEnough(Orders_ID) == false)
        {
            Public.Msg("error", "错误信息", "订单产品库存不足", false, "{back}");
        }
    }
...
<div class="content_div">
  <table width="100%" border="0" cellpadding="0" cellspacing="0" class="content_table">
    <tr>
      <td class="content_title">订单[<%=Orders_SN%>]发货信息</td>
    </tr>
    <tr>
      ...
      <form id="Form1" name="frm_delivery" method="post" action="orders_do.aspx?orders_id=<%=Orders_ID%>">
			...
			<tr>
			  ...
			  <tr>
				...
				<input type="text" name="Orders_Delivery_companyName" id="Orders_Delivery_companyName" value="<%=Orders_Delivery_Name %>" /> <%=myApp.Delivery_Company_Select("Orders_Delivery_company", "onchange=\"$('#Orders_Delivery_companyName').val($(this).val());\"")%></td>
			  </tr>
			  <tr>
