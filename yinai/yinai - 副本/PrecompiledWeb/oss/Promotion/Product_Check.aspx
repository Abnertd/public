...
<script runat="server">
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        ReqURL = "keyword=" + Server.UrlEncode(defaultkey) + "&product_cate=" + product_cate + "&target=" + target + "&tag=" + tag;
    }
</script>
...
<script type="text/javascript">
	function MM_ReturnID(){ //v3.0
	    ...
	    async: false,
        type: "POST",
        url: "/promotion/picker_do.aspx?action=saveproductid&timer=" + Math.random(),
        data: "productid=" +  $("#selarrow").val()
        });
	    $('#product_picker').load("/promotion/picker_do.aspx?action=showproduct&group=<%=group %>&target=<%=target %>&limit=<%=limit %>&timer=" + Math.random());
  ...
  <table width="600" border="0" cellpadding="0" cellspacing="0">
        ...
        <table width="100%" border="0" cellspacing="0" cellpadding="5">
		  ...
		  <tr bgcolor="#F5F9FC" >
			...
			<span id="main_cate"><%=myApp.Product_Category_Select(product_cate, "main_cate")%></span>
			 <input type="text" name="keyword" size="50" id="keyword" onfocus="if(this.value=='输入商品编码、商品名称进行搜索'){this.value='';}" value="<% =keyword %>"> <input type="submit" name="btn_sch" class="btn_01" id="btn_sch" value="搜索"  onclick="refresh_productlist();"/>
    ...
    <tr>
      <td id="productlist" height="360" valign="top">
        ...
        <script type="text/javascript">
        jQuery("#list").jqGrid({
        url: '/promotion/picker_do.aspx?action=productlist&<%=ReqURL %>',
			datatype: "json",
