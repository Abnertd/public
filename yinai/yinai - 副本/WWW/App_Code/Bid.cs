...
public class Bid
{
    ...
    public Bid()
    {
        ...
        tender = TenderFactory.CreateTender();
        ...
        if (Type == 0)
        {
            if (entitys != null)
            {
                foreach (BidProductInfo entity in entitys)
                {
                    ...
                    Response.Write("<td alt=\"" + entity.Bid_Product_Remark + "\"  title=\"" +entity.Bid_Product_Remark + "\">" + tools.CutStr(entity.Bid_Product_Remark, 5) + "</td>");
                    Response.Write("<td>");
        ...
        if (Type == 0)
        {
            if (entitys != null)
            {
                foreach (BidProductInfo entity in entitys)
                {
                    ...
                    Response.Write("<td>" +tools.CutStr( entity.Bid_Product_Remark,7) + "</td>");
                    Response.Write("<td>");
    ...
    public void SupplierBidEnterList(int Type)
    {
        ...
        if (datatable.Rows.Count > 0)
        {
            ...
            {
                //QueryInfo query = new QueryInfo();
                // MyTender.GetTenders(Query);
                //        Query.ParamInfos.Add(new ParamInfo("AND", "int", "TenderInfo.Tender_IsShow", "=", "1"));

                IList<TenderInfo> entitys = GetTenders(Convert.ToInt32(entity["Bid_ID"]), supplier_id);
                DateTime Bid_BidStartTime = tools.NullDate(entity["Bid_BidStartTime"]);
                DateTime Bid_BidEndTime = tools.NullDate(entity["Bid_BidEndTime"]);
                tmp_list = tmp_list + "<tr>";
                tmp_list = tmp_list + "<td title=" + entity["Bid_Title"]+ supplier_id + entity["Bid_ID"] + ">" + tools.CutStr(entity["Bid_Title"].ToString(), 18) + "</td>";
                tmp_list = tmp_list + "<td  title=" + entity["Bid_MemberCompany"] + ">" + tools.CutStr(entity["Bid_MemberCompany"].ToString(), 18) + "</td>";
                //tmp_list = tmp_list + "<td>" + Bid_BidStartTime.ToString("yyyy-MM-dd HH:mm:ss") + "至" + Bid_BidEndTime.ToString("yyyy-MM-dd HH:mm:ss") + "</td>";
                tmp_list = tmp_list + "<td>" + Bid_BidStartTime.ToString("yyyy-MM-dd") + "<span style=\"color:#ff6600\">至</span>" + Bid_BidEndTime.ToString("yyyy-MM-dd") + "</td>";
                ...
                else
                {
                    if (entitys!=null)
                    {
                        int count = entitys.Where(p => p.Tender_IsWin == 1).Count();
                        tmp_list = tmp_list + "<td>";
                        tmp_list += count < 0 ? "未中标" : "已中标" + "</td>";
                    }
                    else
                    {
                        tmp_list = tmp_list + "<td>已结束</td>";
    ...
    public void Tender_Member_List(IList<Glaer.Trade.B2C.Model.BidInfo> list, int Type)
    {
        ...
        //{
        //    sb.Remove(sb.ToString().IndexOf('0'),2);
        //}
        //sb.Remove(sb.ToString().IndexOf('0')>=0? sb.ToString().IndexOf('0'):0, 2);
