...
public class Member
{
    ...
    private static void Tender_Management(int main, int sub, StringBuilder sb)
    {
        ...
        sb.Append("<li " + (main == 1 && sub == 3 ? " class=\"on\"" : "") + "><a href=\"/member/tender_sign.aspx\">管理招标</a></li>");
                    ...
                    if (ordersinfo.Orders_Status == 2)
                    {
                        ...
                        if (entitys != null)
                        {
                            foreach (OrdersGoodsInfo entity in entitys)
                            {
                                ...
                                if (Goods_Type == 0 || Goods_Type == 3 || (Goods_Type == 2 && entity.Orders_Goods_ParentID > 0))
                                {
                                    MyProduct.UpdateProductSaleAmount(Product_ID, (int)Math.Round(Goods_Amount));
                                }
                            }
                        }
                        entitys = null;
    ...
    public string ReplaceOrdersContract_New(OrdersInfo ordersInfo, int contact_no)
    {
        ...
        string MemberSupplierCorporate = "";
        string MemberSupplierAdress = "";
        if (ordersInfo != null)
        {
            ...
            if (memberInfo != null)
            {
                ...
                if (memberInfo != null)
                {
                    ...
                    MemberSupplierCorporate = Supplierinfo.Supplier_Corporate;
                    MemberSupplierAdress = Supplierinfo.Supplier_Address;
        ...
        if (ContractTemplateEntity != null)
        {
            ...
            if (supplierInfo != null)
            {
                ...
                Contract_Template_TopContent = Contract_Template_TopContent.Replace("{time}",string.IsNullOrEmpty(Time)?"卖家尚未确认订单":Time) ;
        ...
        //合同模板运输责任
        ContractTemplateInfo ContractTemResponsibleEntity = MyContract.GetContractTemplateBySign("Sell_Contract_Template_Responsible", pub.CreateUserPrivilege("d4d58107-0e58-485f-af9e-3b38c7ff9672"));
        ...
        if (ordersInfo != null)
        {
            ...
            if (supplier != null)
            {
                ...
                if (ContractInfo != null)
                {
                    ...
                    {
                        Template_Html = Template_Html.Replace("{supplier_name}", supplierInfo.Supplier_CompanyName);
                        Template_Html = Template_Html.Replace("{supplier_corproate}", supplierInfo.Supplier_Corporate);
                        Template_Html = Template_Html.Replace("{supplier_adress}", supplierInfo.Supplier_Address);
                    }
                    if (memberInfo != null)
                    {
                        Template_Html = Template_Html.Replace("{member_name}", MemberSupplierCompanyName);
                        Template_Html = Template_Html.Replace("{member_corproate}", MemberSupplierCorporate);
                        Template_Html = Template_Html.Replace("{member_adress}", MemberSupplierAdress);
                    }
                    ...
                    Template_Html = Contract_Template_TopContent + Template_Html + ordersInfo.Orders_ContractAdd + ContractTemResponsible + ContractTemOnlyRead_Content + ContractTemOnlyEndContent;
                ...
                foreach (DataRow RdrList in DT.Rows)
                {
                    ...
                    if (entitys != null)
                    {
                        ...
                        foreach (var entity in entitys)
                        {
                            ...
                            if (i1 == 1)
                            {
                                Orders_Delivery_Goods_ProductAmount = (int)Math.Round( entity.Orders_Delivery_Goods_ProductAmount);
                            }
                        }
                    }
                    if (i % 2 == 0)
        ...
        try
        {
            ...
            if (DT != null)
            {
                foreach (DataRow RdrList in DT.Rows)
                {
                    ...
                    //计算当前收货单--发货数量
                    double OrdersDeliveryProductSum = tools.CheckFloat(DBHelper.ExecuteScalar("select sum(Orders_Delivery_Goods_ProductAmount)  from Orders_Delivery_Goods where Orders_Delivery_Goods_DeliveryID=(select Orders_Delivery_ID from Orders_Delivery where Orders_Delivery_ID=" + Orders_Delivery_ID + ")").ToString());
                    ...
                    // 2:签收金额
                    double ReceiveSum = tools.CheckFloat(DBHelper.ExecuteScalar("select SUM(Orders_Delivery_Goods_ProductAmount*Orders_Delivery_Goods_ProductPrice) from Orders_Delivery_Goods where  Orders_Delivery_Goods_DeliveryID=" + Orders_Delivery_ID + "").ToString());
    ...
    private void Achieve_Confirm_Receipt()
    {
        ...
        #region 支付
        //获取本单已结算签收金额
        double totalreceiveprice = tools.NullDbl(DBHelper.ExecuteScalar("select sum(Orders_Delivery_Goods_ProductAmount*Orders_Delivery_Goods_ProductPrice) as totalprice from Orders_Delivery inner join Orders_Delivery_Goods on Orders_Delivery_Goods_DeliveryID=Orders_Delivery_ID where Orders_Delivery_ReceiveStatus=2 and Orders_Delivery_OrdersID=" + ordersinfo.Orders_ID));
        //获取本发货单签收金额
        double curreceiveprice = tools.NullDbl(DBHelper.ExecuteScalar("select sum(Orders_Delivery_Goods_ProductAmount*Orders_Delivery_Goods_ProductPrice) as totalprice from Orders_Delivery inner join Orders_Delivery_Goods on Orders_Delivery_Goods_DeliveryID=Orders_Delivery_ID where Orders_Delivery_ID=" + ODEntity.Orders_Delivery_ID));
        //获取本发货单签收佣金金额
        double CommissionPrice = tools.NullDbl(DBHelper.ExecuteScalar("select sum(Orders_Delivery_Goods_ProductAmount*Orders_Delivery_Goods_brokerage*Orders_Delivery_Goods_ProductPrice) as totalprice from Orders_Delivery inner join Orders_Delivery_Goods on Orders_Delivery_Goods_DeliveryID=Orders_Delivery_ID where Orders_Delivery_ID=" + ODEntity.Orders_Delivery_ID));
        CommissionPrice = Math.Round(CommissionPrice, 2);
        //结算金额超过采购总金额
        if (totalreceiveprice + curreceiveprice > ordersinfo.Orders_Total_AllPrice)
        {
